 ...
 * 
 */
@Service(value = "amsArchivesAuditService")
public class AmsArchivesAuditServiceImpl extends MybatisBaseServiceImpl implements AmsArchivesAuditService {
	...
	public List<Map<String, Object>> getAuditUserGroupList(Map<String, Object> parmMap) {
		...
		return amsArchivesAuditDao.getAuditUserGroupList(parmMap);
	}
	...
	public void confirmHandAmsArchivesAudit(String[] auditIds, String handUserId, String handType) {
		// 组转参事
		Map<String, Object> parmMap = new HashMap<String, Object>();
		// 档案id集合
		parmMap.put("auditIds", auditIds);
		// 审核状态
		...
		amsArchivesAuditDao.handAmsArchivesAudit(parmMap);
		// 处理档案
		// 返回结果集,申请档案id//此处变更档案状态
		Map<String, Object> result1 = new HashMap<String, Object>();
		result1 = selectHandleAms(auditIds);
		...
		if (handType.equals("1")) {
			messageInfo.setContent(resultUser.getFullname() + "审批通过了" + auditIds.length + "份档案申请，请留意！");
			messageInfo.setTitle("审批通过");
			messageInfo.setIsPush(1);
		} else if (handType.equals("2")) {
			messageInfo.setContent(resultUser.getFullname() + "审批拒绝了" + auditIds.length + "份档案申请，请留意！");
			messageInfo.setTitle("审批拒绝");
			messageInfo.setIsPush(1);
		} else if (handType.equals("3")) {
			messageInfo.setContent(resultUser.getFullname() + "执行了" + auditIds.length + "份档案申请任务，请留意！");
		...
		amsList = amsArchivesAuditDao.selectHandleAms(parmMap);// 得到档案集合
	 ...
	 * @param handType
	 */
	public void confirmAmsArchivesAudit(String[] auditIds, String handUserId, String handType) {
		// 组转参事
		Map<String, Object> parmMap = new HashMap<String, Object>();
		// 档案id集合
		parmMap.put("auditIds", auditIds);
		...
		AdminUser resultUser = adminUserService.selectObject(parmMapu);
		if (handType.equals("1")) {
			messageInfo.setContent(resultUser.getFullname() + "审批通过了" + auditIds.length + "份档案申请");
			messageInfo.setTitle("审批通过");
			messageInfo.setIsPush(1);
		} else if (handType.equals("2")) {
			messageInfo.setContent(resultUser.getFullname() + "审批拒绝了" + auditIds.length + "份档案申请");
			messageInfo.setTitle("审批拒绝");
			messageInfo.setIsPush(1);
		} else if (handType.equals("3")) {
			messageInfo.setContent(resultUser.getFullname() + "执行了" + auditIds.length + "份档案申请任务");
			messageInfo.setTitle("执行任务");
			messageInfo.setIsPush(0);
		}
		messageInfo.setCtime(new Date());
	 ...
	 * @param handType
	 */
	public String confirmCommitTaskAmsArchivesAudit(String[] auditIds, String handUserId, String handType,
		 ...
		 * amsArchivesAuditDao.commitTaskUpdayeAms(parmMaps);
		 */
		// 调用中信接口，根据申请id进行相应的查询得到
		if (Config.ZX_IS_STAR.equals("1")) {
			List<Map<String, String>> param = null;
			param = amsArchivesAuditDao.getOtherAPIparam(parmMap);
			try {
				res = HttpRequestUtil.sendPostRequest(Config.ZX_API_URL + "/szdag/api/otherapi/AuditConfirm",
	 ...
	 *            0.存档 1.取档
	 */
	public void addAmsArchivesAudits(String creator, String userId, String type, String[] archivesIds,
			Map<String, Object> parmMaps) {
		// 组转参事
		Map<String, Object> parmMap = new HashMap<String, Object>();
		// 档案id集合
		...
		// 调用dao层插入数据
		amsArchivesAuditDao.addAmsArchivesAudits(parmMap);
	...
	public List<Map<String, Object>> selectTaskInfo(String userId, String status) {
		...
		// 通道档案
		List<Map<String, Object>> galleryList = amsArchivesAuditDao.selectTaskInfo(parmMap);
		// 库房档案
		List<Map<String, Object>> storeAmss = amsArchivesAuditDao.selectTaskStoreNum(parmMap);
		// 库区档案
		List<Map<String, Object>> areaAmss = amsArchivesAuditDao.selectTaskStoreAreaNum(parmMap);
	...
	public Map<String, Object> selectTaskExecute(Map<String, Object> parmMap) {
		...
		parmMap.put("offset", (Integer.parseInt(String.valueOf(parmMap.get("pageIndex"))) - 1)
				* Integer.parseInt(String.valueOf(parmMap.get("pageSize"))));// 第几页（起始量）
		parmMap.put("rows", Integer.parseInt(String.valueOf(parmMap.get("pageSize"))));// 每页条数（偏移量）
		result.put("finishedNum", amsArchivesAuditDao.pageCount(finishedNumparm));
		result.put("willExecuteNum", amsArchivesAuditDao.pageCount(willExecuteNumparm));
		result.put("amsList", amsArchivesAuditDao.selectTaskExecute(parmMap));
		return result;
	}
	...
	public Map<String, Object> selectTaskSubmit(Map<String, Object> parmMap) {
		...
		List<Object> auditIds = new ArrayList<Object>();
		List<Object> archivesIdsIn = new ArrayList<Object>();// 存档、入库档案集合
		List<Object> archivesIdsOut = new ArrayList<Object>();// 取档、出库档案集合
		// 查询得到通道档案集合
		List<Map<String, Object>> amsList = new ArrayList<Map<String, Object>>();
		amsList = amsArchivesAuditDao.selectTaskExecute(parmMap);
		for (Map<String, Object> list : amsList) {
			auditIds.add(list.get("auditId"));
			if (list.get("auditType").equals("0")) {// 0.存档 1.取档
				archivesIdsIn.add(list.get("archivesId"));
			} else if (list.get("auditType").equals("1")) {
				archivesIdsOut.add(list.get("archivesId"));
			}

		}
		result.put("auditIds", auditIds);
	...
	public Map<String, Object> selectHandleAms(String[] parmMap) {
		...
		param.put("auditIds", parmMap);
		// 查询得审核档案集合
		List<Map<String, Object>> amsList = new ArrayList<Map<String, Object>>();
		amsList = amsArchivesAuditDao.selectHandleAms(param);
		// 出库取档档案id，入库存档档案id
	...
	public Integer confirmTaskCancel(String[] split) {
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("auditIds", split);
		param.put("status", "-1");
		// 更新档案---恢复到之前的状态
		amsArchivesDao.taskCancelUpdate(param);
		// 更新申请单
		amsArchivesAuditDao.updates(param);
		return null;
	}
	...
	public List<Map<String, Object>> getTaskList(Map<String, Object> parmMap) {
		...
		try {
			taskList = amsArchivesAuditDao.getTaskList(parmMap);
			List<Map<String, Object>> taskListNum = new ArrayList<Map<String, Object>>();// 总数
			taskListNum = amsArchivesAuditDao.getTaskListNum(parmMap);
	...
	public List<Map<String, Object>> getApplyAmsList(Map<String, Object> parmMap) {
		...
		parmMap.put("offset", (Integer.parseInt(String.valueOf(parmMap.get("pageIndex"))) - 1)
				* Integer.parseInt(String.valueOf(parmMap.get("pageSize"))));// 第几页（起始量）
		parmMap.put("rows", Integer.parseInt(String.valueOf(parmMap.get("pageSize"))));// 每页条数（偏移量）
		taskAmsList = amsArchivesAuditDao.getApplyAmsList(parmMap);
		return taskAmsList;
	}
	...
	public List<Map<String, Object>> getGalleryList(String userId, String status, String applyNo) {
		...
		// 通道档案
		List<Map<String, Object>> galleryList = amsArchivesAuditDao.selectTaskGallaryList(parmMap);
		// 库房档案
		List<Map<String, Object>> storeAmss = amsArchivesAuditDao.selectTaskStoreNums(parmMap);
		// 库区档案
		List<Map<String, Object>> areaAmss = amsArchivesAuditDao.selectTaskStoreAreaNums(parmMap);
		for (Map<String, Object> storeAms : storeAmss) {
			Map<String, Object> storeData = new HashMap<String, Object>();
			storeData.put("storeId", storeAms.get("strore_id"));
			storeData.put("storeName", storeAms.get("storeName"));
				...
				if (areaAms.get("strore_id").equals(storeAms.get("strore_id"))) {
					...
					for (Map<String, Object> galleryData : galleryList) {
						if (galleryData.get("storeId").equals(storeAms.get("strore_id"))
							...
							gallery.put("storeLr", galleryData.get("storeLr"));
							gallery.put("auditIds", galleryData.get("auditId"));
							galleryDatas.add(gallery);
						}
					}
					areaData.put("galleryData", galleryDatas);
	...
	public List<Map<String, String>> getAmsArchiveAudit() {
		List<Map<String, String>> list=amsArchivesAuditDao.getAmsArchiveAudit();
		return list;
	}

	@Override
	public Map<String, String> getAmsArchiveAuditCount() {
		Map<String, String> count=amsArchivesAuditDao.getAmsArchiveAuditCount();
		return count;
	}
    /**
     * 档案出入库信息统计
    ...
    public List<Map<String, String>> getArchivesAuditCount(){
    	List<Map<String, String>> count=amsArchivesAuditDao.getArchivesAuditCount();
