// 初始化插件
var openWinNum;// 默认填充窗口类型1，2，4
var iChannelIDs = new Array();// 默认通道号数组
var chanalFixNos = new Array();// 自定义通道号数组
// 全局保存当前选中窗口
var g_iWndIndex = 0; // 可以不用设置这个变量，有窗口参数的接口中，不用传值，开发包会默认使用当前选择窗口
$(function () {
	// 检查浏览器
	var u = navigator.userAgent.toUpperCase();
	if(u.indexOf('TRIDENT') == -1){// Trident代表IE 如果不是IE浏览器将直接不加载视频插件
		alert("你的浏览器内核不支持查看视频，若要查看视频请更换主流浏览器，如IE，360等");
		return false;
	}
	// 检查插件是否已经安装过
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
		...
		// 单次双选弹框
            var dblChoseAlert = simpleAlert({
                "content":"您还未安装插件，请立即安装？",
                "buttons":{
                    "安装":function () {
                        dblChoseAlert.close();
                        location.href=config.getVideoplugin;
                    },
                    "取消":function () {
                    	var dblChoseAlert1 = simpleAlert({
                            "content":"取消安装将不能正常播放视频，是否安装？",
                            "buttons":{
                                "安装":function () {
                                    dblChoseAlert1.close();
                                    dblChoseAlert.close();
                                    location.href=config.getVideoplugin;
                                },
                                "不安装":function () {
	...
	if(openWinNum==undefined||openWinNum==null||openWinNum==""){
		openWinNum=2;
	}
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin("100%", "100%", {
        iWndowType: openWinNum,
  ...
  // 获取指定展示在首页的视频通道号
    	Ajax.custom({
    		url : config.getDeviceChanal,
    		data : {

    		},// 传递参数
    		type : 'GET'
    	},function(data) {
    		for (var i = 0; i < starOpenChanal; i++) {
    			chanalFixNos[i]=data.data[i].chanal
    			}
    	});
});
...
function clickLogin() {
	...
	Ajax.custom({
		...
		if(data.data!=null){
			if(data.data.loginip!=null){
				szIP=data.data.loginip;
				$("#loginip").val(szIP)
			}
			if(data.data.loginport!=null){
				szPort=data.data.loginport;
				$("#port").val(szPort);
			}
			if(data.data.loginname!=null){
				szUsername=data.data.loginname;
				$("#username").val(szUsername);
			}
			if(data.data.loginpwd!=null){
				szPassword=data.data.loginpwd;
...
// 开始预览
function clickStartRealPlay(chanal) {
    ...
    if(chanal!=null&&chanal!=undefined&&chanal!=""){
    		iChannelID=chanal;
    }
...
// 批量预览 WndNum批量窗口数量,默认预览摄像头
function plStartRealPlay(WndNum){
	for(var i=0;i<WndNum;i++){
		var oWndInfo = WebVideoCtrl.I_GetWindowStatus(i),
		szIP = $("#ip").val(),
		iStreamType = parseInt($("#streamtype").val(), 10),
		iChannelID = parseInt($("#channels option").val(), 10),
		bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,
		szInfo = "";
	...
	if(chanalFixNos.length>0){// 若给定了摄像图通道号，就按照给定的通道号预览
		if(i>=chanalFixNos.length){
			return;
		}
		iChannelID=chanalFixNos[i];
	}else{
		if(i>=iChannelIDs.length){
			return;
		}
		iChannelID=iChannelIDs[i];
	}
	var iRet = WebVideoCtrl.I_StartRealPlay(szIP, {
		iWndIndex:i,
		iStreamType: iStreamType,
...
function clickStartPlayback(szStartTime,szEndTime) {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		...
		iRet = -1;
	if(szStartTime==null||szStartTime==undefined||szStartTime==""){
		szStartTime = $("#starttime").val();
	}
	if(szEndTime==null||szEndTime==undefined||szEndTime==""){
		szEndTime = $("#endtime").val();
	}
	if ("" == szIP) {
		return;
