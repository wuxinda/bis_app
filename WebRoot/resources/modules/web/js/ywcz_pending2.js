...
function toexcel(){
	...
	window.location.href=config.getAuditAmsListToexcel+'.do?archiveName='+archiveName+'&timeStart='+timeStart+'&timeEnd='+timeEnd+'&store='+store+'&storeArea='+storeArea+'&status='+status+'&type='+type+'&inout=1'
}
function loadStore(){
	//初始化库房
	$.ajax({
...
function jdqr(archivesId,auditInId,status){
	Ajax.custom({
		url : config.updateArchivesStatus,
		data : {
...
function jdsc(archivesId,auditInId,status){
	Ajax.custom({
		url : config.updateArchivesStatus,
		data : {
			archivesId:archivesId,
...
function jdmnrfid(archivesId,auditInId,status){
	Ajax.custom({
		url : config.updateArchivesStatus,
		data : {
			archivesId:archivesId,
