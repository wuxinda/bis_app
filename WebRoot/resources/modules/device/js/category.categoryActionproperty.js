...
function form_submit() {
	...
	$('input[name=actionpropertyIds]').val(propertyIds);
...
function refreshListingProperty() {
	...
	DataGridDataSource.prototype = {
		...
		data : function(options, callback) {
			var url = BASE_URL + '/deviceActionproperty/page';
			var self = this;
				...
				dataSource : new DataGridDataSource({
					...
					formatter : function(items) {
						$.each(items, function(index, item) {
							item.checkbox = '<input type="checkbox" name="post[]" id="select_single_p_' + item.actionpropertyId + '" class="select-single" data-type="p" value="'
									+ item.actionpropertyId + '">';
						});
					}
				}),
