...
function form_cancel() {
	// history.go(-1);
	window.location.href = BASE_URL + "/deviceBrand/index";
}
...
function form_submit() {
	...
	$('input[name=categoryIds]').val(propertyIds);
...
function refreshListingProperty() {
	...
	DataGridDataSource.prototype = {
		...
		data : function(options, callback) {
			var url = BASE_URL + '/deviceCategory/page';
			var self = this;
				...
				dataSource : new DataGridDataSource({
					...
					formatter : function(items) {
						$.each(items, function(index, item) {
							item.checkbox = '<input type="checkbox" name="post[]" id="select_single_p_' + item.categoryId + '" class="select-single" data-type="p" value="'
									+ item.categoryId + '">';
