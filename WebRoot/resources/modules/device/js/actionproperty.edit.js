...
function form_submit() {
	var actionpropertyId = $("input[name=actionpropertyId]").val();

	$(".input-submit").attr('disabled', true);

	var saveCallBack;
	if (actionpropertyId == '' || actionpropertyId == 0) {
...
function form_cancel() {
	window.location.href = BASE_URL + "/deviceActionproperty/index";
}
...
function form_save_added(data, textStatus) {
	if (data.status === 0) {
		...
		if (back_listing == true) {
			...
			window.location.href = BASE_URL + "/deviceActionproperty/add";
		}
	} else {
		alert(data.msg);
	}
