...
function form_submit() {
	...
	var actlogId = $("input[name=actlogId]").val();

	$(".input-submit").attr('disabled', true);

	var saveCallBack;
	if (actlogId == '' || actlogId == 0) {
...
function form_cancel() {
	window.location.href = BASE_URL + "/amsArchivesActlog/index";
}
...
function form_save_added(data, textStatus) {
	if (data.status === 0) {
		...
		if (back_listing == true) {
			...
			window.location.href = BASE_URL + "/amsArchivesActlog/add";
		}
	} else {
		alert(data.msg);
	}
