		...
		<section class="vbox">
			<header class="header bg-white b-b clearfix bmViewHeader">
				<div class="row m-t-sm">
					<form class="form-inline" action="${BASE_URL}/deviceCategory/categoryActionproperty" id="searchForm">
			...
			<section class="scrollable wrapper w-f">
				<form class="form-horizontal" id="edit_form" action="${BASE_URL}/deviceCategory/categoryActionproperty" method="post">
					...
					<input type="hidden" id="categoryId" name="categoryId" value="${categoryId}" /> <input type="hidden" name="actionpropertyIds" value="${actionpropertyIds}" />
				</form>
			</section>
		...
		<section class="vbox">
			...
			<section class="scrollable w-f">
				<ul class="nav nav-stacked list-group gutter list-group-lg list-group-sp sortable">
					<c:forEach items="${actionpropertys}" var="v">
						<li class="b-b m-t-none-reset" id="li_property_${v.actionpropertyId}" property_id="${v.actionpropertyId}" draggable="true"><a href="javascript:;"> <i title="移除该属性" class="fa fa-times pull-right m-t-xs fa-remove-brand"></i> <i class="fa fa-fw fa-ellipsis-v"></i><font class="property-name">${v.name}</font> </a>
						</li>
					</c:forEach>
				</ul>
			</section>
...
<script src="${STATIC_URL}/modules/device/js/category.categoryActionproperty.js" type="text/javascript"></script>